var S3_resalt_Scene = cc.Scene.extend({
  onEnter: function(){
    this._super();
     size = cc.director.getWinSize();
    var s3_resalt_layer = new S3_resalt();
    this.addChild(s3_resalt_layer);
  }
});

var s3_chenge_flg = false;

var S3_resalt = cc.Layer.extend({
...
audio_engin = cc.audioEngine;
    ...
    var stage_rank_num = Math.floor((stage3_r1_rank + stage3_r2_rank + stage3_r3_rank) / 3);
    ...
    if (stage3_rank != null) {
      if (stage1_rank < stage_rank_num) {
        stage3_rank = stage_rank_num;
      }
    }else {
      stage3_rank = stage_rank_num;
    }
    cc.eventManager.addListener(touchListener_s3_resalt, this);
...
var touchListener_s3_resalt = cc.EventListener.create({
